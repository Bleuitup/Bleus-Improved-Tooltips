...
with no argument asks the armor nodes' `GetHasTech()`. Lose the arms lab and both go false in the
same tech-tree update, so the icon is hidden and then colored red.
...
`GetHasTech`, so armor genuinely drops without a working arms lab. Hiding the icon was not a lie —
...
there is no arms lab and the upgrade was ever researched, restore the icon's visibility and color.
...
- Vanilla has already set the red by the time the hook runs, since its color block is after its
...
- Build a second arms lab: icons return to normal color, no stale red.
...
- Weapons and armor independently — one researched, the other not.
