...
--                   HIVE PANEL: up to two research slots, each a research notification in
--                   miniature. Those live in ImprovedTooltips_HiveResearchSlots.lua.
--
-- Both are created for every row, and the mode picks which one shows, so switching mode in the
-- Mods panel applies at once.
--
...
Script.Load("lua/ImprovedTooltips/ImprovedTooltips_HiveResearchSlots.lua")
...
local function CreateResearchIcon(slot)
	...
	slot.itResearchDna:SetTexture(IT.kBuildMenuTexture)
...
function GUIHiveStatus:CreateStatusContainer(slotIdx, locationId)
	...
	if not slot or not slot.background or not ResolveIcons() then
		return
	end
	...
	if IT.kShowHiveResearch then
		CreateResearchIcon(slot)
		IT.CreateHiveResearchSlots(slot)
	end
...
function GUIHiveStatus:UninitializeStatusSlot(slotIdx)
	...
	if slot then
		...
		IT.DestroyHiveResearchSlots(slot)
...
function GUIHiveStatus:UpdateStatusSlot(slotIdx, slotData)
	...
	IT.UpdateHiveResearchSlots(slot, state, visible and not ringMode)

	if slot.itResearchRing then

		local researching = visible and ringMode and IT.GetHiveIsResearching(state)
